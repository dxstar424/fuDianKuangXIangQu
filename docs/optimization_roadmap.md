...
| [**sprint_strategy_0711.md**](./sprint_strategy_0711.md) | **7/11–7/15 少次多阶段冲刺（S1–S4）** |
| [dcu_decode_benchmark_interpretation.md](./dcu_decode_benchmark_interpretation.md) | **DCU decode 访存实测解读（gfx936）** |
| [**scoring_and_results_interpretation.md**](./scoring_and_results_interpretation.md) | **评分公式 · 历次平台实测复盘 · 排名估算** |
...
| 对比项 | 榜首（豆包F4） | **本队 lutinayi（7/10）** | 他队参考（富贵花开，**非本队**） |
| ---------- | ----------- | -------------- | --------- |
| 最终得分 | 89.2 | **59.97** | 84.7 |
| 8K-16K 吞吐 | 19.51 | **10.04** | 16.65 |
| 4K-8K 吞吐 | 21.42 | 12.92 | 18.37 |
| 16K-32K 吞吐 | 15.05 | 5.77 | 13.49 |
| SLA 扣分 | 0 | 0 | 0 |
| 精度扣分 | 0 | 0 | 0 |
...
| 时间节点 | 目标分数 | 关键动作 |
| ---------- | ---- | ------------------------------------- |
| 7/11 | **≥65（S1）** | recover launch 平台 1 次；见 sprint_strategy_0711 |
| 7/12 | **≥12 tok/s @8-16K（S2）** | `ab_stage2`：eager-off / 单档 warmup |
| 7/13 | **≥14 tok/s @8-16K（S3）** | GQA 深接 + quick gate |
| 7/14–7/15 | 冲 80+ / 冻结 | S4 Graph 可选；7/15 上午只修编译 |


### 1.3 团队提交策略

- **统一用本队竞赛平台账号提交**（**不是**排行榜「富贵花开」）；`lutinayi_branch` / `main` 仅门禁通过后更新。
- **每次平台提交前**必须由 **G** 跑过 `gate_check.sh quick`（至少）或 `full`（大合并 / KV FP8 后）。
- **合并与 Git 由 I 负责**：合并前打 tag，跑不通立即回滚。
- 反面案例：他队盲提交掉分；本队 7/10 接近 Baseline（59.97），须先去掉负优化再冲。
...
Phase 0  baseline 数据 + profiling 基线
   ↓
Phase 1 / S1  recover launch（0.94/eager/stock）── 当前默认
   ↓
S2  launch 单变量（ENFORCE_EAGER / 单档 warmup）
   ↓
S3  GQA decode 深接（唯一 Phase2 主刀）── 已接线 selector wrap
   ↓
S4  HIP Graph opt-in（须 S3 净增 + ≥48h）
   ↓
（默认不做）KV FP8 / FlashQLA 整库 / defrag
   ↓
Phase 6  文档量化贡献 + 平台提交（≤4 次）
```
...
### 5.2 竞赛平台实测（本队 lutinayi · 2026-07-10）


| 档位     | TTFT P99 | TPOT P99 | 吞吐 tok/s | vs 榜首 | SLA  |
| ------ | -------- | -------- | -------- | ----- | ---- |
| 4-8K   | —        | —        | 12.92    | -8.50 | 0 扣分 |
| 8-16K  | —        | —        | 10.04    | -9.47 | 0 扣分 |
| 16-32K | —        | —        | 5.77     | -9.28 | 0 扣分 |


### 5.3 平台得分记录


| 日期         | 来源 | 8K-16K | 4K-8K | 16K-32K | 最终得分  | SLA | 精度扣分 | 备注       |
| ---------- | ---- | ------ | ----- | ------- | ----- | --- | ---- | -------- |
| 2026-07-10 | **本队 lutinayi_branch** | 10.04 | 12.92 | 5.77 | **59.97** | 0 | 0 | **本队当前正式分** |
| 2026-07-06 | 他队·富贵花开（参考） | 16.65  | 18.37 | 13.49   | 84.74 | 0   | 0    | **非本队** |
| 2026-07-08 | 他队·LUX.（参考） | 13.96  | 15.68 | 10.02   | 77.27 | 0   | 0    | **非本队** |
