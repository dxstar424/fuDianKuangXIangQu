...
| 项目 | 最终状态 |
|---|---|
| 实现候选锚点 | `3d0c500d5b129dce19f91a142fbe8c99e4f4037c`（`perf: bundle gfx936 quant kernel into rocm wheel`） |
| GitLab 提交分支 | [`origin/dx_branch`](https://gitlab.eduxiji.net/fudiankuangxiangqu/2025pra-fdu-fudiankuangxiangqu/-/tree/dx_branch) |
| GitHub 提交分支 | [`github/main`](https://github.com/dxstar424/fuDianKuangXIangQu/tree/main) 与 [`github/dx_branch`](https://github.com/dxstar424/fuDianKuangXIangQu/tree/dx_branch) |
| 推荐平台入口 | GitHub 仓库 `https://github.com/dxstar424/fuDianKuangXIangQu`，分支 `main` |
| 最新已评分结果 | **66.7878**；`15.00 / 11.97 / 6.11 tok/s`；SLA 与精度均无扣分 |
| 当前 W8 候选评分 | 尚未进行平台评测；不得把 66.7878 归因于当前 wheel 内置 W8 |
| 最终候选实机状态 | 已有 gfx936 kernel microbenchmark；尚未完成当前整包的 SCNet 端到端吞吐、P99 与精度验证 |
| 本地验证结果与边界 | `tests/fdu` 共 142 项通过，`git diff --check` 通过；macOS 无 DCU、ROCm 与 `hipcc`，不能代替实机 wheel 构建和端到端验证 |

这里的 `3d0c500` 是代码实现锚点；仅修改本文档的后续提交不改变候选实现。最终提交以远程分支 HEAD 为准。66.7878 是唯一可引用的近期平台实测分数；当前 W8 候选的 **66–74 分**只可作为盲测风险区间，不是测量结果，也没有证据承诺 90 分。
...
原运行时 JIT 候选的平台结果为 66.7878，与上一轮 66.8175 在 `0.33%` 内一致。平台没有提供启动日志或 checkout commit，无法判断 W8 是否激活，因此不把它归因为 W8 成绩。最终候选仅消除激活链不确定性：内核编入 wheel、Python 默认 `w8`、自动定位 `_rocm_C`，请求量化但零 layer 激活时停止启动。
...
本轮按最终提交决策不再执行 SCNet。下表是保留的验收门禁，不代表当前整包已经通过；若平台首轮出现 build、启动、SLA 或精度异常，应据此做最小复现，而不是继续叠加 Attention、KV Cache 或 scheduler 改动。
...
1. 提交 GitHub `main`；GitLab `dx_branch` 与 GitHub `dx_branch` 保留同一内容作为追溯与备份。
2. 最终候选只新增 wheel 内置、可失败关闭的 selective W8 路径，并保留 LLMM1、KV block-table 脏提交、prefix cache 和静默日志；不再盲合其他分支的 Attention/KV 实验代码。
3. 66.7878 是保底参照，不是当前候选成绩。当前候选若正确激活且端到端收益能覆盖在线量化与 prefill 还原成本，盲测预期为 66–74 分；若 wheel ABI、GPU smoke 或量化 layer 激活失败，服务会显式停止，风险表现为评测失败而不是静默伪装成优化成功。
4. 8–16K 权重最高且当前吞吐仅 11.97 tok/s，16–32K 为 6.11 tok/s；真正继续提分应优先用平台日志确认这两档的 decode 占比和 W8 命中率。没有日志时，不以修改 block size、替换 Attention 后端或强开 KV FP8 继续下注。
5. 若平台首轮失败，最小回滚是把 `FDU_GFX936_QUANT_MODE` 恢复为 `off`；不要同时修改模型、Attention、KV Cache 和调度参数，以免失去归因能力。
